...
import { useGetReportTemplates } from 'src/actions/report-template';
...
import ReportTemplateTableRow from '../report-template-table-row';
...
export default function ReportTemplateListView() {
  const popover = usePopover();
  const table = useTable();
  const router = useRouter();
  const lastTableDataRef = useRef<any[]>([]);
  const { query, setQuery, debouncedQuery } = useDebouncedSearch(500);
  const { reportTemplates, reportTemplatesMeta, isLoading } = useGetReportTemplates({
    searchFor: 'all',
    page: table.page + 1,
    perPage: table.rowsPerPage,
    searchedValue: debouncedQuery,
  });
  const [tableData, setTableData] = useState<any[]>(reportTemplates);
  const [selectedColumns, setSelectedColumns] = useState<string[]>(DEFAULT_COLUMNS);

  useEffect(() => {
    if (!isLoading && Array.isArray(reportTemplates)) {
      lastTableDataRef.current = reportTemplates;
      setTableData(reportTemplates);
    }
  }, [reportTemplates, isLoading]);

  const notFound = !isLoading && reportTemplatesMeta?.total === 0 && tableData.length === 0;

  const handleEditRow = useCallback(
    (id: number) => {
      router.push(paths.dashboard.reportTemplate.edit(id));
    },
    [router]
  );
    ...
    >
      ...
      >
        ...
        <Button
          ...
          href={paths.dashboard.reportTemplate.new}
          variant="contained"
      ...
      <InfoTable>
        ...
        <TablePaginationCustom
          count={reportTemplatesMeta?.total || tableData.length || 0}
          page={table.page}
