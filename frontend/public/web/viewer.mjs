...
function generateDynamicFilename(originalFilename) {
  const now = new Date();
  ...
  // Try to get a meaningful document name
  let docName = 'document';
  
  // Method 1: Check for originalFilename URL parameter
  try {
    const urlParams = new URLSearchParams(window.location.search);
    const originalParam = urlParams.get('originalFilename');
    if (originalParam && originalParam !== 'undefined' && originalParam.length > 0) {
      // Decode the URL parameter and handle spaces properly
      const decodedName = decodeURIComponent(originalParam);
      // Replace spaces with underscores first, then clean other special chars
      const cleanName = decodedName
        .replace(/\s+/g, '_')  // Replace spaces with underscores
        .replace(/[^a-zA-Z0-9_-]/g, '_')  // Replace other special chars
        .replace(/_{2,}/g, '_')  // Replace multiple underscores with single
        .replace(/^_+|_+$/g, '');  // Remove leading/trailing underscores
      return `${cleanName}_${dateStr}_${timeStr}.pdf`;
    }
  } catch (e) {
  ...
  // Method 2: Use original filename if provided
  if (originalFilename && originalFilename !== 'undefined' && originalFilename.length > 0) {
    docName = originalFilename.replace(/\.[^/.]+$/, "").replace(/[^a-zA-Z0-9_-]/g, '_');
  } else {
    ...
    try {
      ...
      if (fileParam) {
        ...
        if (fileName && fileName.includes('.')) {
          docName = fileName.replace(/\.[^/.]+$/, "").replace(/[^a-zA-Z0-9_-]/g, '_');
        }
      }
    } catch (e) {
      // Fallback to default
    ...
    // Method 4: Try to get from document title
    if (docName === 'document') {
      try {
        if (document.title && document.title !== 'PDF.js viewer') {
          docName = document.title.replace(/[^a-zA-Z0-9_-]/g, '_');
        }
      } catch (e) {
        // Fallback to default
      }
  ...
  return `${docName}_${dateStr}_${timeStr}.pdf`;
}

function download(blobUrl, filename) {
  const dynamicFilename = generateDynamicFilename(filename);
  const a = document.createElement("a");
  if (!a.click) {
...
class DownloadManager {
  ...
  downloadData(data, filename, contentType) {
    ...
    download(blobUrl, dynamicFilename);
  }
